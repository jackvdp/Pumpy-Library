...
public class Artwork {
    ...
    public init(token: String? = nil, store: String? = nil) {
        appleMusicToken = token
        appleMusicStoreFront = store
    }
    
    public func getArtwork(from track: Track, size: Int, completion: @escaping (UIImage?) -> Void) {
...
//        }
    ...
    func getArtworkFromStore(from track: Track, size: Int, completion: @escaping (UIImage) -> Void) {
        if let token = appleMusicToken, let storefront = appleMusicStoreFront {
            let amAPI = ArtworkGateway(token: token, storeFront: storefront)
    ...
    public static func fetchImage(from url: URL, completionHandler: @escaping (_ image: UIImage?) -> ()) {
