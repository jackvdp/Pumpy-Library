...
struct UpNextArtworkView<N:NowPlayingProtocol,P:PlaylistProtocol,B:BlockedTracksProtocol,Q:QueueProtocol,T:TokenProtocol>: View {
    ...
    var body: some View {
        if geo.size.height >= geo.size.width {
            VStack {
                ...
                UpNextView<Q,N,B,T>(fontStyle: .custom(subFont, size: geo.size.width * 0.03 * 0.75), opacity: 0.5, showButton: false)
                    .padding(geo.size.height * 0.05)
            ...
            HStack {
                ...
                VStack {
                    UpNextView<Q,N,B,T>(fontStyle: .custom(subFont, size: geo.size.height * 0.03 * 0.75), opacity: 0.5, showButton: false)
                        .padding(geo.size.height * 0.05)
                }.frame(width: extDisMgr.frameHeight(geo.size.width * 0.5))
            }
        }
